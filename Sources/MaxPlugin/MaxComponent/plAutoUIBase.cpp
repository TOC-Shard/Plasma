...
void plAutoUIBase::AddPickWebMComponentButton(int16_t id, const ST::string& scriptName, const ST::string& name, int vid, std::unordered_set<ST::string> vstates)
{
    ST::string scriptNameNew = !scriptName.empty() ? scriptName : IMakeScriptName(name);
    ...
    fDesc->AddParam(id, ST2M(scriptNameNew), TYPE_INODE, 0, 0,
        p_end,
        p_end);
    plAutoUIParam* param = new plPickWebMComponentButtonParam(id, name);
    param->SetVisInfo(vid, std::move(vstates));
    fParams.push_back(param);
}
